...
** 现有解决方案及其问题
  ...
  这个方案算是基本可行。但还是存在一些问题：
  - 严重依赖字段的命名，字段如果不是以id结尾就不起作用。虽然可以继续加这个关键字列表，但是还是很粗糙，而且可能“误杀”。
  - 



** JSON.parse reviver 参数
** https://github.com/sidorares/json-bigint/blob/master/lib/parse.js 这个库
