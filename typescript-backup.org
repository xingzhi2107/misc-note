...
* 类型体操
  上面讲了TS的类型、以及对象类型的几个基本操作，有了这两个概念之后，再结合TS提供的一些工具类型，我们就可以做一些简单的类型体操。
  ...
  - Pick<Type, Keys> 与 Omit<Type, Keys>，用来提取或者剔除某个类型的key。比如，可以用Pick从Entry衍生出对应的创建表单的类型：Pick<Topic，'title' | 'content' | 'group_id'>
