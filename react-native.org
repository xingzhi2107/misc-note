...
* SafeAreaView不safe
 SafeAreaView在ScrollView里出现的话，貌似会引起SafeAreaView不停的layout，导致元素抖动。具体细节未考究。

 [[https://stackoverflow.com/questions/62046011/view-is-shaking-by-no-reason-on-react-native-ios][View is shaking by no reason on React Native iOS]]

* 监听hardwareBackPress时机不当
  最近实现一个可展开输入框。在用户已经输入文本之后，如果按物理返回键，需要拦截下来询问用户是否确认退出。这自然需要用到监听hardwareBackPress这个事件。问题就出在，当这个输入框没有被聚焦的时候，这个监听依然在起作用。这就导致物理返回键被一直拦截，用户无法使用物理返回键退出输入框所在的页面。

  想了一会之后，觉得改变监听的时机，则很自然的化解这个问题：
  1. 在focus的时候才去注册监听
  2. 在blur或者willUnmount的时候，取消监听

* AndroidInputSoftMode与KeyboardAvoid
  KeybarodAvoid在android下基本是不可用的。在Android下，需要设置AndroidInputSoftMode。一般设置成SOFT_INPUT_ADJUST_RESIZE即可。话说，这个为什么不是默认行为？
  https://github.com/facebook/react-native/issues/11681#issuecomment-277645925

* Keyboard的显示与隐藏
** Tap的时候，不要让TextInput失焦，Keyboard不要隐藏
  默认情况下，当用户点击TextInput以外的区域的时候，会让TextInput失去焦点，并且会让Keybaord隐藏起来。但是有些事时候，我们不希望有这样的默认行为。比如，点击了某个按钮之后会修改TextInput的内容，用户需要继续编辑。

  上面所说的需求，RN是可以实现的。但是前提是这个组件是被ScrollView包裹着的，这样就可以用ScrollView的[[https://reactnative.dev/docs/scrollview#keyboardshouldpersisttaps][keyboardShouldPersistTaps]]属性来控制。看了RN的这个issue的[[https://github.com/facebook/react-native/issues/4229#issuecomment-167069887][回答]]之后，我觉得他说得很有道理，RN应该提供更细粒度的控制能力。总不能每次为了点击不隐藏键盘，都要用一个ScrollView包裹起来吧？目前的这种方案太取巧了。

* Shadow
  RN的Shadow也是坑多。ios跟android的shadow机制也不一样。

  在Android中，RN只能通过elevation属性控制阴影，它与android native的[[https://developer.android.com/develop/ui/views/theming/shadows-clipping][elevation]]是一致的。注意！这个属性还有zIndex效果，数值高了就会悬浮到顶部。

  而在ios中，RN通过如下四个属性控制阴影：

#+begin_src 
          shadowColor: 'black', // 控制阴影的颜色
          shadowOffset: {       // 控制阴影的2d偏移
            width: 0,
            height: 0,
          },
          shadowOpacity: 0.2,   // 第一个坑：默认是0。不设置，阴影就看不到。控制阴影颜色的不透明度，会乘以shadowColor的不透明度。
          shadowRadius: 15,     // 控制阴影的模糊度
          background: 'white',  // 第二个坑：要给元素设置一个背景色，不然阴影也会看不到。
#+end_src

* zIndex
* 字体
** 如何使用系统字体？
  网络上有如何使用自定义字体的教程，但是我们今天要在ios使用系统的SF Pro。试了半天，SF Pro不行，全称也不行，感觉只能写成undefined，让它直接采用系统默认字体（也是SF Pro）。这种行为就很奇怪，有点无法理解。
